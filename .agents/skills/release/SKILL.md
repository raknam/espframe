...
description: >-
  ...
  release", "tag a release", "start the release action", or wants firmware
...
If the user supplied a tag, use it. If not, infer the next patch version from
the latest stable `vX.Y.Z` tag and get confirmation before publishing.
...
### 3. Verify the Release Action Started
