...
This page describes touch gestures on the Espframe: wake, sleep, and advance slideshow. The frame uses a **GSL3680** capacitive touchscreen. On the slideshow screen you can wake the display, turn it off with a timed hold, and advance to the next photo.
...
| **Double tap** | — | Advances to the **next** photo in the slideshow. |
...
- Touch is handled by LVGL widgets on the slideshow and portrait-pair views (`on_press`, `on_short_click`, `on_release`). The same gestures apply whether a single image or a portrait pair is shown. Double-tap is detected when two short taps occur within 400 ms.
